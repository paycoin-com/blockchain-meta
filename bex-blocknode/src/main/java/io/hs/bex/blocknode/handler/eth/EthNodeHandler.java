package io.hs.bex.blocknode.handler.eth;
...
@Service( "ETH-NodeHandler" )
@Scope("prototype")
public class EthNodeHandler implements GenericNodeHandler
{

    // ---------------------------------
    private static final Logger logger = LoggerFactory.getLogger( EthNodeHandler.class );
    // ---------------------------------
    ...
    public Node init( NodeProvider nodeProvider )
    {
        ...
        host = env.getProperty( "node.eth" + prefix + ".host" );
        
        node.setProvider( nodeProvider );
        node.setId( nodeProvider.getId());
        node.setName( "Ethereum-" + nodeProvider.getNetworkType().name().toLowerCase() );
